            ...
            if (t1 != null)
            {
                ...
                else if (t1 is AlphaNumUnderscoreToken)
                {
                    ...
                    if (t2 != null)
                    {
                        if (t2 is ColonToken)
                        {
                            return new IdentfierLexToken(t1.Value()); // throw away the colon
                        }
                        else
                        {
                ...
                else if (t1 is AlphaNumToken)
                {
                    ...
                    if (t1Val.Equals("if"))
                    {
                        ...
                        {
                            _lexTokenStack.Push(new IdentfierLexToken(elseLabel.Value()));
                            _lexTokenStack.Push(new ElseStatementLexToken(elseStatement.Value()));
                            _lexTokenStack.Push(new IdentfierLexToken(thenLabel.Value()));
                            ...
                            {
                                _lexTokenStack.Push(new IdentfierLexToken(operand2.Value()));
                            }
                            _lexTokenStack.Push(new BinaryOperationLextToken(operation.Value()));
                            _lexTokenStack.Push(new IdentfierLexToken(operand1.Value()));
